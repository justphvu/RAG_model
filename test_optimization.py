...
def test_embedding_optimizations():
    ...
    # Initialize optimized embedding model with reduced batch size
    ...
    # Test batch processing with smaller batch
    logger.info("Testing batch processing...")
    texts = [
        "Каковы требования к поступающим?",
        "Сколько стоит программа?",
        "Каковы сроки подачи заявок?",
        "Расскажите мне об учебной программе"
    ]
...
def test_conversation_management():
    ...
    # Initialize conversation manager with reduced history
    conv_manager = ConversationManager(
        max_conversations=10,  # Reduced from 100
    ...
    # Add fewer test messages
    conv_manager.add_user_message(user_id, "Каковы требования к поступающим?")
    conv_manager.add_assistant_message(user_id, "Требования к поступающим включают в себя...")
    conv_manager.add_user_message(user_id, "Сколько это стоит?")
    conv_manager.add_assistant_message(user_id, "Стоимость программы...")
    
    # Get conversation history
    history = conv_manager.get_conversation_history(user_id, max_messages=4)
    logger.info(f"Conversation history length: {len(history)}")
    
    # Get context window with reduced size
    context = conv_manager.get_context_window(user_id, max_tokens=500)  # Reduced from 1000
...
def test_enhanced_rag_pipeline():
    ...
    # Test queries (reduced number)
    test_queries = [
        "Каковы требования к поступающим на факультет компьютерных наук?",
        "Сколько стоит магистерская программа?",
        "Расскажите мне о программах аспирантуры!"
    ]
    ...
    for i, query in enumerate(test_queries):
        ...
        try:
            ...
            # Clear GPU cache after each query
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
...
def test_performance_monitoring():
    ...
    # Initialize performance monitor with reduced history
    monitor = PerformanceMonitor(
        max_history_size=20,  # Reduced from 100
    ...
    # Get average metrics for shorter duration
    avg_metrics = monitor.get_average_metrics(duration_minutes=0.5)  # Reduced from 1
    logger.info(f"Average metrics (30 sec): {avg_metrics}")
