...
class OptimizedEmbeddingModel:
    ...
    def __init__(
        ...
        cache_dir: str = Constants.EMBEDDING_CACHE_DIR,
        batch_size: int = Constants.BATCH_SIZE,
        enable_disk_cache: bool = Constants.ENABLE_DISK_CACHE,
        use_half_precision: bool = Constants.USE_HALF_PRECISION  # Enable half precision by default
    ):
    ...
    def embed_single_cached(self, text: str, prompt_name: str) -> List[float]:
        """
        Embed a single text with caching and memory optimizations.
        ...
        # Generate new embedding with memory optimizations
    ...
    def embed_batch(self, texts: List[str], prompt_name: str) -> List[List[float]]:
        """
        Embed multiple texts efficiently using batching and memory optimizations.
        ...
        # Process in batches with memory optimizations
        with torch.inference_mode(), torch.cuda.amp.autocast(enabled=self.use_half_precision):
